...
def identifyPores(colorGrid, width, height, myColor):
    ...
    for x in range(width):
        for y in range(height):
            color = colorGrid[x, y]
            if ((color[0] > rgbThreshold) and (color[1] > rgbThreshold) and (color[2] > rgbThreshold)):
                nrPores += 1
...
def boxCounting(colorGrid, width, height, size, threshold, myColor):
        ...
        for j in range(nrYBoxes):
            ...
            for dx in range(size):
                for dy in range(size):
                    ...
                    y = j * size + dy
                    color = colorGrid[x, y]
                    if color == myColor: nrPoresPixels += 1
            if (nrPoresPixels > nrPixelsThreshold):
                nrOccupiedBoxes += 1
    ...
    if (nrOccupiedBoxes > 0):
        ...
        D = 0
    print("box size =", size, "pores =", nrOccupiedBoxes, " L =", format(L, '.3f'), " D =", format(D, '.3f'))
...
def main():
    ...
    picture.show()
    plt.xlabel('Box size', fontsize=16, labelpad=5)
    plt.ylabel('N', fontsize=16, labelpad=5)
    plt.ion()
    size = 200    #pixels
    threshold = 0.2
    while (size >= 1):
